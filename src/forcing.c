...
// [1] 施加边界条件，包括单元和河流分段的边界条件
#if defined(_CVODE_CUDA)
void ApplyBc(forc_struct *forc, elem_struct *elem, river_struct *river, pihm_struct_d pihm_d, int t)
{
	/* Element boundary conditions */
	if (forc->nbc > 0)
	{
		ApplyElemBc(forc, elem, pihm_d, t);
	}

	/* River boundary condition */
	if (forc->nriverbc > 0)
	{
		ApplyRiverBc(forc, river, pihm_d, t);
	}
}
...
// [2] 施加驱动力条件,包括气象和LAI
#if defined(_NOAH_)   /* Noah Model */
#if defined(_CVODE_CUDA)
void ApplyForc(forc_struct *forc, elem_struct *elem, pihm_struct_d pihm_d, int t, int rad_mode,
...
#else                /* is_sm_et Model */
#if defined(_CVODE_CUDA)
void ApplyForc(forc_struct *forc, elem_struct *elem, pihm_struct_d pihm_d, int t)
#else
...
#if defined(_NOAH_)
#if defined(_CVODE_CUDA)
	ApplyMeteoForc(forc, elem, pihm_d, t, rad_mode, siteinfo);
#else
    ApplyMeteoForc(forc, elem, t, rad_mode, siteinfo);
#endif
#else
#if defined(_CVODE_CUDA)
	ApplyMeteoForc(forc, elem, pihm_d, t);
#else
	ApplyMeteoForc(forc, elem, t);   // 施加气象条件
#endif
#endif
...
#else
#if defined(_CVODE_CUDA)
	ApplyLai(forc, elem, pihm_d, t);      // 施加LAI条件
#else
...
// [2.1]
#if defined(_CVODE_CUDA)  /* CUDA */
void ApplyElemBc(forc_struct *forc, elem_struct *elem, pihm_struct_d pihm_d, int t)
{
	int             i, k;
	...
	for (i = 0; i < nelem; i++)
	{
		...
		for (j = 0; j < NUM_EDGE; j++)
		{
			if (elem[i].attrib.bc_type[j] > 0)
			{
				...
				pihm_d->elem_d_bc_head[j][i] = forc->bc[ind].value[0];
			}
			else if (elem[i].attrib.bc_type[j] < 0)
			{
				ind = -elem[i].attrib.bc_type[j] - 1;
				pihm_d->elem_d_bc_flux[j][i] = forc->bc[ind].value[0];
			}
		}  // for (j = 0; j < NUM_EDGE; j++)
	}  // for (i = 0; i < nelem; i++)
...
// [2.2]
#if defined(_CVODE_CUDA)
void ApplyRiverBc(forc_struct *forc, river_struct *river, pihm_struct_d pihm_d, int t)
{
	int             i, k;
...
# pragma omp parallel for
	...
	for (i = 0; i < nriver; i++)
	{
		...
		if (river[i].attrib.riverbc_type > 0)
		{
			...
			pihm_d->river_d_bc_head[i] = forc->riverbc[ind].value[0];  // CPU -> GPU
		}
		else if (river[i].attrib.riverbc_type < 0)
		{
			ind = -river[i].attrib.riverbc_type - 1;
			pihm_d->river_d_bc_flux[i] = forc->riverbc[ind].value[0];  // CPU -> GPU
		}
	}
}
...
#if defined(_NOAH_)
#if defined(_CVODE_CUDA)
void ApplyMeteoForc(forc_struct *forc, elem_struct *elem, 
					pihm_struct_d pihm_d,   // CUDA
					int t, int rad_mode,
	                const siteinfo_struct *siteinfo)
#else
...
#else
#if defined(_CVODE_CUDA)
void ApplyMeteoForc(forc_struct *forc, elem_struct *elem, pihm_struct_d pihm_d, int t)
#else
...
# pragma omp parallel for
    ...
    {
        IntrplForc(&forc->meteo[k], t, NUM_METEO_VAR);   // 时间插值
    }
...
# endif
            ...
            {
				IntrplForc(&forc->rad[k], t, 2);   /* 时间插值太阳辐射值 */
            }
        }

        /* Calculate Sun position for topographic solar radiation */
        SunPos(siteinfo, t, &spa);  // from topo_radn.c, 计算太阳的位置
    }
#endif
...
# pragma omp parallel for
    ...
    for (i = 0; i < nelem; i++)
    {
...
#if defined(_CVODE_CUDA)
		ind = pihm_d->elem_d_attrib_meteo_type[i] - 1;

		// 拷贝到CUDA空间的elem结构体中去.     // CPU -> GPU
		pihm_d->elem_d_wf_prcp[i] = forc->meteo[ind].value[PRCP_TS] / 1000.0;
		pihm_d->elem_d_es_sfctmp[i] = forc->meteo[ind].value[SFCTMP_TS];
		pihm_d->elem_d_ps_rh[i] = forc->meteo[ind].value[RH_TS];
		pihm_d->elem_d_ps_sfcspd[i] = forc->meteo[ind].value[SFCSPD_TS];
		pihm_d->elem_d_ef_soldn[i] = forc->meteo[ind].value[SOLAR_TS];
		pihm_d->elem_d_ef_soldn[i] = (elem[i].ef.soldn > 0.0) ? elem[i].ef.soldn : 0.0;
#if defined(_NOAH_)
		pihm_d->elem_d_ef_longwave[i] = forc->meteo[ind].value[LONGWAVE_TS];  // 长波辐射
#endif
		pihm_d->elem_d_ps_sfcprs[i] = forc->meteo[ind].value[PRES_TS];
#if defined(_NOAH_)
		...
		if (rad_mode > 0)
		{
			if (forc->nrad > 0)
			{
				pihm_d->elem_d_ef_soldir[i] = forc->rad[ind].value[SOLDIR_TS];
				pihm_d->elem_d_ef_soldif[i] = forc->rad[ind].value[SOLDIF_TS];
			}
			pihm_d->elem_d_ef_soldn[i] = TopoRadn(&elem[i].topo, elem[i].ef.soldir,  // 太阳辐射变化
...
#else
#if defined(_CVODE_CUDA)
void ApplyLai(forc_struct *forc, elem_struct *elem, pihm_struct_d pihm_d, int t)
#else
...
#if defined(_CVODE_CUDA)
		if (pihm_d->elem_d_attrib_lai_type[i] > 0)
		{
			ind = elem[i].attrib.lai_type - 1;

			pihm_d->elem_d_ps_proj_lai[i] = forc->lai[ind].value[0];     // CPU -> GPU
		}
		else
		{
			pihm_d->elem_d_ps_proj_lai[i] = MonthlyLai(t, elem[i].attrib.lc_type);    // CPU -> GPU
		}
#else
    ...
    else
    {
        ...
        while (first <= last)
        {
            ...
            if (t >= ts->ftime[middle - 1] && t <= ts->ftime[middle])
            {
                ...
                {
                    ts->value[j] =
                        ((realtype)(ts->ftime[middle] - t) *
                        ts->data[middle - 1][j] +
                        (realtype)(t - ts->ftime[middle - 1]) *
                        ts->data[middle][j]) /
                        (realtype)(ts->ftime[middle] - ts->ftime[middle - 1]);
                }
                break;
            }
...
realtype MonthlyLai(int t, int lc_type)
{
    /*
...
#ifndef _CVODE_CUDA
realtype MonthlyRl(int t, int lc_type)
{
    /*
...
realtype MonthlyMf(int t)
{
    pihm_t_struct   pihm_time;

	const realtype          mf_tbl[12] = {
