...
#if defined(_OPENMP)
void RunTime(realtype start_omp, realtype *cputime, realtype *cputime_dt)
{
    static realtype   ptime_omp;
    realtype          ct_omp;

    ptime_omp = (ptime_omp == 0.0) ? start_omp : ptime_omp;
    ct_omp = omp_get_wtime();
    *cputime_dt = (realtype)(ct_omp - ptime_omp);
    *cputime = (realtype)(ct_omp - start_omp);
    ptime_omp = ct_omp;
}
#else
void RunTime (clock_t start, realtype *cputime, realtype *cputime_dt)
{
    ...
    ct = clock();
    *cputime_dt = ((realtype)(ct - ptime)) / CLOCKS_PER_SEC;
    *cputime = ((realtype)(ct - start)) / CLOCKS_PER_SEC;
    ptime = ct;
}
#endif
