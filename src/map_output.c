        ...
        if (prtvrbl[i] != 0)
        {
            switch (i)
            {
                ...
                case SNOWH_CTRL:
					...
					/*   MM-PIHM-0.10 µÄNoahÄ£ÐÍÉÐÎ´¿¼ÂÇ±ù´¨ºÍ±ù¸ÇÄ£¿é  ~Li Jian
    ...
    int upd_intvl, int nvar, varctrl_struct *varctrl)
{
    ...
    varctrl->nvar = nvar;
    varctrl->var = (const realtype **)malloc(nvar * sizeof(realtype *));
    varctrl->buffer = (realtype *)calloc(nvar, sizeof(realtype));
    varctrl->counter = 0;
}
    ...
    int intr, int upd_intvl, int nvar, int nnode, varctrl_struct *varctrl)
{
    ...
    varctrl->x = (realtype *)malloc(nnode * sizeof(realtype));
    varctrl->y = (realtype *)malloc(nnode * sizeof(realtype));
    varctrl->zmin = (realtype *)malloc(nnode * sizeof(realtype));
    varctrl->zmax = (realtype *)malloc(nnode * sizeof(realtype));
    if (intr == ELEMVAR)
    {
