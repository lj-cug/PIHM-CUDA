...
realtype KrFunc(realtype beta, realtype satn)
{
    return sqrt(satn) *
...
realtype FieldCapacity(realtype beta, realtype kv, realtype smcmax,
    realtype smcmin)
{
    /*
    ...
    realtype          denom;
    realtype          df;
    realtype          dsatn = 999.0;
    realtype          mx;
    realtype          ftheta;
    int             n;
    realtype          smcref;
    const realtype    KFC = 5.79E-9;
    const realtype    TOL = 1.0E-3;
...
realtype WiltingPoint(realtype smcmax, realtype smcmin, realtype alpha, realtype beta)
{
    const realtype    PSIW = 200.0;

    return 0.5 * (smcmax - smcmin) *
        pow(1.0 / (1.0 + pow(PSIW * alpha, beta)), 1.0 - 1.0 / beta) + smcmin;
}

int SoilTex(realtype silt, realtype clay)
{
    /*
 ...
 */
realtype PtfKv(realtype silt, realtype clay, realtype om, realtype bd, int topsoil)
{
    realtype          kv;

    /* Calculate Kv in cm/day */
    kv = exp(7.755 + 0.0352 * silt + 0.93 * (realtype)topsoil -
        0.967 * bd * bd - 0.000484 * clay * clay - 0.000322 * silt * silt +
        0.001 / silt - 0.0748 / om - 0.643 * log(silt) -
        0.01398 * bd * clay - 0.1673 * bd * om +
        0.02986 * (realtype)topsoil * clay - 0.03305 * (realtype)topsoil * silt);
...
realtype PtfThetas(realtype silt, realtype clay, realtype om, realtype bd, int topsoil)
{
    realtype          thetas;
    ...
    thetas = 0.7919 + 0.001691 * clay - 0.29619 * bd -
        ...
        0.0001664 * (realtype)topsoil *silt;

    return thetas;
}

realtype PtfThetar(realtype silt, realtype clay)
{
    realtype          thetar;
...
realtype PtfAlpha(realtype silt, realtype clay, realtype om, realtype bd, int topsoil)
{
    realtype          alpha;

    /* Calcualte alpha in cm */
    alpha = exp(-14.96 + 0.03135 * clay + 0.0351 * silt + 0.646 * om +
        15.29 * bd - 0.192 * (realtype)topsoil - 4.671 * bd * bd -
        0.000781 * clay * clay - 0.00687 * om * om + 0.0449 / om +
        0.0663 * log(silt) + 0.1482 * log(om) - 0.04546 * bd * silt -
        0.4852 * bd * om + 0.00673 * (realtype)topsoil * clay);
...
realtype PtfBeta(realtype silt, realtype clay, realtype om, realtype bd, int topsoil)
{
    realtype          beta;
