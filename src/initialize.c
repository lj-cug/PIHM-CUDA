...
#if defined(_CVODE_CUDA)
#if defined(_NOAH_)
void Initialize(pihm_struct pihm, pihm_struct_d pihm_d, N_Vector CV_Y)
#else
...
#endif
{
...
#else
    pihm->elem = (elem_struct *)malloc(nelem * sizeof(elem_struct));      // element struct on CPU  AOS-style
#endif
    pihm->river = (river_struct *)malloc(nriver * sizeof(river_struct));  // river struct on CPU  AOS-style
...
#if defined(_NOAH_)
#if defined(_CVODE_CUDA)
		/* Noah模型需要在初始化的时候施加初始驱动力 
		   注意：此时还没有分配CUDA数组空间
		*/
		/*
		ApplyForc(&pihm->forc, pihm->elem, pihm_d, pihm->ctrl.starttime,
    ...
    /* Initialize state variables 
	   从文件读取数据后，初始化状态变量
	   IN: pihm->elem, pihm->river;  OUT: CV_Y
	*/
    InitVar(pihm->elem, pihm->river, CV_Y);  // set NVector CV_Y on GPU 
...
# pragma omp parallel for
    ...
    for (i = 0; i < nelem; i++)
    {
        ...
        realtype          x[NUM_EDGE];
        realtype          y[NUM_EDGE];
        realtype          zmin[NUM_EDGE];
        realtype          zmax[NUM_EDGE];
        realtype          distx;
        realtype          disty;
...
realtype _WsAreaElev(int type, const elem_struct *elem)
{
    realtype          ans = 0.0;
    int             i;
...
{
    int             i;
    const realtype    INIT_UNSAT = 0.1;
#if defined(_FBR_)
    const realtype    INIT_FBR_GW = 5.0;
#endif
...
void InitVar(elem_struct *elem, river_struct *river, N_Vector CV_Y)
{
	...
	realtype *InitCV_Y = NULL;
	InitCV_Y = (realtype *)malloc(size * sizeof(realtype));
#endif
    
    // 初始化主机的用户数组 CV_Y_CPU !!!
	/* Initialize the Userdata for meshed basin */
//#if defined(_OPENMP)
//# pragma omp parallel for
//#endif
    ...
    for (i = 0; i < nelem; i++)
    {
...
#if defined(_CVODE_CUDA)
		InitCV_Y[SURF(i)] = elem[i].ic.surf;
		InitCV_Y[UNSAT(i)] = elem[i].ic.unsat;
		InitCV_Y[GW(i)] = elem[i].ic.gw;
#else
        ...
        elem[i].ws0 = elem[i].ws;   // elem.ws0 was used in infil.c
    }

	/* Initialize the Userdata for river */
//#if defined(_OPENMP)
//# pragma omp parallel for
//#endif
    for (i = 0; i < nriver; i++)
...
#if defined(_CVODE_CUDA)
		InitCV_Y[RIVSTG(i)] = river[i].ic.stage;
		InitCV_Y[RIVGW(i)] = river[i].ic.gw;
#else
		NV_Ith(CV_Y, RIVSTG(i)) = river[i].ic.stage;
		NV_Ith(CV_Y, RIVGW(i)) = river[i].ic.gw;
#endif
      //  river[i].ws0 = river[i].ws;   // river[i].ws0 was not used.
    }
...
#if defined(_CVODE_CUDA)
// 将主机上的用户数据拷贝到设备CV_Y上，仅1次
	set_nvector_cuda(CV_Y, InitCV_Y, size);
	free(InitCV_Y);
#endif
}
