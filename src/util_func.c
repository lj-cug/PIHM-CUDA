...
void set_nvector_cuda(N_Vector vec, realtype * data, int size)
{
	realtype* vec_host_ptr = N_VGetHostArrayPointer_Cuda(vec);
	for (int i = 0; i < size; i++) {
		vec_host_ptr[i] = data[i];  // 具体的从主机到设备的数据拷贝
	}
	N_VCopyToDevice_Cuda(vec);
}

/* Copy Vector from Device to Host */
void get_nvector_cuda(N_Vector vec, realtype * data, int size)
{
	N_VCopyFromDevice_Cuda(vec);
	realtype* vec_host_ptr = N_VGetHostArrayPointer_Cuda(vec);
	for (int i = 0; i < size; i++) {
		data[i] = vec_host_ptr[i]; // 具体的从设备到主机的数据拷贝
	}
}
#endif